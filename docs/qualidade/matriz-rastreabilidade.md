...
| 16 | **Teste de carga dos três caminhos críticos** (F12b) | `GET /missoes/proximas`, `POST /carteira/transferencias`, `POST /webhooks/transportadora` (endpoint removido depois, ADR 0031) | `tools/carga/carga.js` (k6 v2.2.0), 3 cenários × 5 min | ✅ | **14.967 requisições, 0 respostas 5xx.** Radar a 74,6 req/s com p95 de 4,3 ms e sem joelho; 1.205 transferências na MESMA carteira sem deadlock; os três tetos de rate limit medidos batendo com os configurados. Evidência: [`f21-carga.md`](../evidencias/f21-carga.md) |
| 17 | **Diagrama de confiabilidade do modelo de risco** | — | — | ⛔ | **REMOVIDO em 2026-08-30.** O modelo previa falha de entrega e saiu com a extensão logística ([ADR 0031](../adr/0031-remocao-da-extensao-logistica.md)), junto do treino no `verify` e de `modelo-previsao.md`. As métricas que este item registrava (Brier 0,1485 contra 0,1798 do chute constante) foram medidas e ficaram no histórico do ADR 0022 |
...
| 24 | **Sistema inteligente de apoio à decisão** | **Sem implementação desde 2026-08-30.** O modelo de risco previa falha de entrega e foi removido com a extensão logística ([ADR 0031](../adr/0031-remocao-da-extensao-logistica.md)). É perda declarada de escopo: preferimos registrá-la a manter um modelo treinado em dados sintéticos sem nenhum consumidor |
